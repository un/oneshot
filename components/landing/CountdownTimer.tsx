...
export function CountdownTimer({ targetDate, label = "Submission Deadline" }: CountdownTimerProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <p className="text-sm uppercase tracking-wider text-cyber-green-400/80">{label}</p>
...
function TimeUnit({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className="text-xs uppercase tracking-wider text-cyber-orange-400/60">{label}</span>
    </div>
  );
}